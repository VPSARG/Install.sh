#!/bin/bash
#sudo apt-get update  &>/dev/null
#sudo apt-get wget -y &>/dev/null
rm -rf /etc/localtime &>/dev/null
ln -s /usr/share/zoneinfo/America/Mexico_City /etc/localtime &>/dev/null
rm $(pwd)/$0 &> /dev/null
### CONFIGURAR POR 22 SSH
#grep -v "^Port 22" /etc/ssh/sshd_config > /tmp/ssh && mv /tmp/ssh /etc/ssh/sshd_config &>/dev/null
#echo "Port 22" >> /etc/ssh/sshd_config
### COLORES Y BARRA 
msg () {
BRAN='\033[1;37m' && VERMELHO='\e[31m' && VERDE='\e[32m' && AMARELO='\e[33m'
AZUL='\e[34m' && MAGENTA='\e[35m' && MAG='\033[1;36m' &&NEGRITO='\e[1m' && SEMCOR='\e[0m'
 case $1 in
  -ne)cor="${VERMELHO}${NEGRITO}" && echo -ne "${cor}${2}${SEMCOR}";;
  -ama)cor="${AMARELO}${NEGRITO}" && echo -e "${cor}${2}${SEMCOR}";;
  -verm)cor="${AMARELO}${NEGRITO}[!] ${VERMELHO}" && echo -e "${cor}${2}${SEMCOR}";;
  -azu)cor="${MAG}${NEGRITO}" && echo -e "${cor}${2}${SEMCOR}";;
  -verd)cor="${VERDE}${NEGRITO}" && echo -e "${cor}${2}${SEMCOR}";;
  -bra)cor="${VERMELHO}" && echo -ne "${cor}${2}${SEMCOR}";;
  "-bar2"|"-bar")cor="${VERMELHO}======================================================" && echo -e "${SEMCOR}${cor}${SEMCOR}";;
 esac
}
clear
 msg -bar2
 msg -ama "     [ SCRIPT-ARG \033[1;97m 🔹 Edit @Pablo_Ezekiel 🔹\033[1;33m ]"
 echo -e  "\033[1;97m               ACTIVANDO -  ACTUALIZADORES  \033[1;34m "
 msg -bar2
## Script name
SCRIPT_NAME=vpsargup
## Install directory
WORKING_DIR_ORIGINAL="$(pwd)"
INSTALL_DIR_PARENT="/usr/local/vpsargup/"
INSTALL_DIR=${INSTALL_DIR_PARENT}${SCRIPT_NAME}/
## /etc/ config directory
mkdir -p "/etc/vpsargup/"
## Install/update
if [ ! -d "$INSTALL_DIR" ]; then
	echo -e  "\033[1;97m           Instalando Paquetes Especiales"
	echo "          ================"
	sleep 2
	mkdir -p "$INSTALL_DIR_PARENT"
	cd "$INSTALL_DIR_PARENT"
    wget https://github.com/VPS-ARG/zzupdatemaster/blob/master/zzupdate.default.conf -O /usr/local/vpsargup/vpsargup.default.conf  &> /dev/null
	#chmod +x /usr/local/vpsargup/vpsargup.default.conf 
	rm -rf /usr/local/vpsargup/vpsargup.sh
    wget https://github.com/VPS-ARG/zzupdatemaster/blob/master/zzupdate.sh -O /usr/local/vpsargup/vpsargup.sh &> /dev/null
	chmod +x /usr/local/vpsargup/vpsargup.sh
	rm -rf /usr/bin/vpsargup
    wget https://github.com/VPS-ARG/zzupdatemaster/blob/master/zzupdate.sh -O /usr/bin/vpsargup &> /dev/null
	chmod +x /usr/bin/vpsargup
	echo -e  "\033[1;97m              Copiando Instalador Interno "
	
	echo "           --------------------------------"	
	
	msg -bar2
	sleep 2
else
	echo ""
fi

ubu16_fun () {
    wget -O /etc/apt/sources.list https://github.com/VPS-ARG/Repositorio/blob/master/16.04/sources.list &> /dev/null
	echo -e "\033[1;97m SELECCIONO “ UBUNTU 16"
}

ubu18_fun () {
    wget -O /etc/apt/sources.list https://github.com/VPS-ARG/Repositorio/blob/master/18.04/sources.list &> /dev/null
	echo -e "\033[1;97m SELECCIONO “ UBUNTU 18"
}	

otro_fun () {
    
	echo "OK SELECCIONO “ OTRO"
}
	echo -e "\033[1;97m           --¿QUE UBUNTU ESTA UTILIZANDO?--"
	echo -e "\033[1;97m  Digite solo el numero segun respuesta: "
    msg -bar
    echo -e "\033[1;97m Escoja la opcion deseada."
    msg -bar
    echo " 1).- Ubuntu 16.04 "
    echo " 2).- Ubuntu 18.04 "
    echo " 3).- Otro "
	msg -bar
	echo -n "Digite solo el numero segun respuesta: "
    read opcao
    case $opcao in
    1)
    ubu16_fun 
    ;;
    2)
    ubu18_fun
    ;;
    3)
    otro_fun
    ;;
    esac
	
sleep 3
## Restore working directory
cd $WORKING_DIR_ORIGINAL
vpsargup
